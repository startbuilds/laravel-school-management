        ...
        if(isset($fee->status) && $fee->status == 0){
            ...
            if(isset($payment)){
                if ($payment['status'] !== 'success') {
                    ...
                    Toastr::error(__('msg_something_went_wrong'), __('msg_error'));
    ...
    public function callback()
    {
        try {
            ...
            if ($status == 'successful') {
                ...
                Toastr::success(__('msg_your_payment_successful'), __('msg_success'));

            } elseif ($status == 'cancelled') {
                
                Toastr::error(__('msg_your_payment_cancelled'), __('msg_error'));
